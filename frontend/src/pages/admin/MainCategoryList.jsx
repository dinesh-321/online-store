...
export const MainCategoryList = () => {
  ...
  const handleEdit = async (product) => {
    try {
      const res = await axios.get(`/api/admindata/GetmainCategoryData/${product}`);
      const data = res.data;

      MySwal.fire({
        title: <p>Edit Main Category</p>,
  ...
  const EditForm = ({ initialData }) => {
    ...
    //     }
    //     return await axios.put(`/api/admindata/updateCategory/${initialData._id}`, formData);
    //   },
    //   onSuccess: () => {
    ...
    if (typeof file === "string") {
      ...
      try {
        await axios.put(
          `/api/admindata/updatemainCategory/${initialData._id}`,
          formData
        );
        Swal.fire("Updated!", "Category updated successfully.", "success");
        queryClient.invalidateQueries(["maincategoryListkey"]);
  ...
  const deleteAirframe = async ({ deleteid }) => {
    ...
    const response = await axios.delete(
      `/api/admindata/dldmainCategory/${deleteid}`
    );
  ...
  return (
    <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
      <div className="w-full md:p-10 p-4">
        <h2 className="pb-4 text-lg font-medium">Main Category List</h2>
